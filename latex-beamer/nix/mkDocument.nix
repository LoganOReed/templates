...
pkgs.stdenvNoCC.mkDerivation rec {
  ...
  SOURCE_DATE_EPOCH="${SOURCE_DATE_EPOCH}" latexmk ${toString flags} ${inputName}.tex

  runHook postBuild
  '';

  # install command is from coreutils
  # -m644 sets correct permissions
  installPhase = ''
  runHook preInstall

  install -m644 -D *.pdf $out/${name}.pdf
